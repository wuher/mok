...
/**
 * creates a function stub and corresponding expectation into the object
 */
var createStubFunction = exports.createStubFunction = function (funcname, obj) {
    // create expect if it doesn't exist
    if (!obj.hasOwnProperty("expect") || typeof obj.expect !== "object") {
        obj.expect = {};
    }
    // create stub functions for the mock and for the expectation
    obj.expect[funcname] = expectationFunctionStub(funcname);
    obj[funcname] = functionStub(funcname, obj.expect[funcname]());
    expectations.push(obj.expect[funcname]());
};



/**
 * @param name name of the function (used only for more meaningful
 * error messages).
...
var expectationFunctionStub = function (name) {
    ...
    // number of times "withParameters()" has been invoked for this mock func
...
var objectStub = function (obj, recurse) {
    ...
    for (o in obj) {
        try {
            // it seems that when mocking java objects you run into
            // situations where a property is part of an object, yet
